    ...
    public static class TileService
    {
        ...
        public static List<ILayer> CreateTileLayers(string mbTilesPath, string onlineUrl, string cachePath)
        {
            ...
            if (!hasOnline)
            {
                ...
                {
                    // Restrict country polygons to overview zooms (zoom <= 7) to avoid evaluating geometry at city levels
                    landLayer.MinVisible = Helpers.CoordinateHelper.ZoomLevelToResolution(7);
                    landLayer.MaxVisible = double.MaxValue;
        ...
        private static Layer? CreateLandLayer()
        {
            try
            {
                ...
                return new Layer("BaseMap_Land")
                {
                    ...
                    // Land shapes stay visible at all zoom levels
                };
            }
            catch (Exception ex)
            {
