...
namespace PelcoControlNM
{
    public partial class frmMap : Form
    {
        ...
        private void InitializeProviderDropdown()
        {
            ...
            // Hook for the built-in Ctrl+RightClick Area Selection Downloader
            map.OnDownloadStarted += async (bbox, minZ, maxZ, token) =>
            {
                Console.WriteLine($"[ElevationManager] Hook triggered for bbox {bbox.MinLat:F4},{bbox.MinLon:F4} to {bbox.MaxLat:F4},{bbox.MaxLon:F4}");
        ...
        private void MapPanel_MapClicked(object sender, MapClickedEvent e)
        {
            ...
            Invoke((MethodInvoker)delegate
            {
                tooltip.SetText(map, $"Lat: {lastPoint.Lat:F4}\nLon: {lastPoint.Lon:F4}\nAlt: {lastElevation:F1}m");
            });
        }
        ...
        private async void buttonDownload_Click(object sender, EventArgs e)
        {
            ...
            {
                MinLat = _currentCenterLat - 0.15,
                MaxLat = _currentCenterLat + 0.15,
                MinLon = _currentCenterLon - 0.20,
                MaxLon = _currentCenterLon + 0.20
            };
