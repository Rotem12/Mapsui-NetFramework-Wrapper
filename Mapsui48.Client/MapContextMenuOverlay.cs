...
namespace Mapsui48.Client
{
    ...
    public class MapContextMenuOverlay : Form
    {
        ...
        public Color AccentColor { get; set; } = Color.FromArgb(0, 229, 255);
        ...
        public void ShowAt(Point screenPoint, double latitude, double longitude)
        {
            ...
            SetWindowPos(this.Handle, HWND_TOPMOST, x, y, totalWidth, totalHeight, SWP_SHOWWINDOW);
        ...
        {
            if (!force && (DateTime.UtcNow - _showTime).TotalMilliseconds < 350)
            {
                // Debounce: Ignore close requests that arrive within 350ms of opening
                return;
            }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            // 1. Dark Glass Tactical Panel Body
            using (var bgBrush = new SolidBrush(Color.FromArgb(240, 16, 22, 32)))
            {
                g.FillRectangle(bgBrush, rect);
            }

            // 2. Glowing Panel Border
            using (var borderPen = new Pen(Color.FromArgb(140, AccentColor), 1.2f))
            {
            ...
            if (ShowCoordinateHeader)
            {
                ...
                using (var headBrush = new SolidBrush(Color.FromArgb(180, 148, 163, 184))) // Slate-400
                ...
                using (var linePen = new Pen(Color.FromArgb(40, 255, 255, 255), 1f))
                {
            ...
            for (int i = 0; i < visibleItems.Count && i < _buttonRects.Length; i++)
            {
                ...
                Color itemAccent = item.AccentColor ?? AccentColor;
                ...
                using (var btnPath = new GraphicsPath())
                {
                    ...
                    if (isHovered)
                    {
                        using (var glowBrush = new PathGradientBrush(btnPath))
                        {
                            ...
                            glowBrush.CenterColor = Color.FromArgb(200, 30, 48, 68);
                            glowBrush.SurroundColors = new[] { Color.FromArgb(240, 20, 32, 46) };
                            g.FillPath(glowBrush, btnPath);
                        }

                        using (var glowPen = new Pen(itemAccent, 1.8f))
                        {
                    ...
                    {
                        using (var btnBrush = new SolidBrush(Color.FromArgb(160, 24, 33, 47)))
                        using (var btnPen = new Pen(Color.FromArgb(70, itemAccent), 1f))
                        {
